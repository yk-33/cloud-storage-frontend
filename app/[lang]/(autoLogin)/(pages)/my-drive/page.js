    ...
    const EmptyComponent = (pageName) => {
        ...
        return (
            <Stack sx={{alignItems: 'center'}}>
                <Image
                    src={emptyPageImageUrl['myDrive']}
                    alt=""
                    width={200}
                    height={200}
                />
                <Typography variant="h5"  fontFamily={'"Noto Sans"'} sx={{mt: '15px'}}>
                    {t('A place for all of your files')}
                </Typography>
            </Stack>
        )
    }
...
export default function Home() {
            ...
            [
                {
                    ...
                    action: async (id, name) => {
                        ...
                        processActionResponse(res, dispatch, {
                            ...
                            450: { message: t('Can not delete folder that is not existing'), action: 2 },
                        })
                    },
                }
            ],
    ...
    const handleConfirmRename = async()=>{
        ...
        processActionResponse(res, dispatch, {
            200: { message: t(`Renaming ${renameItemType} successful`), action: 1 },  //++++++++++++++++
            450: { action: 2 },
            409: { action: 1 },
        })
    ...
    useEffect(() => {
        // console.log(folderStructor)
        // console.log(folderSelectValue)
        if (folderSelectValue === null) {  // 此时folderStructor也为null，还在登录的状态
            return
        }
        updateFileList()
    ...
    return (
        <>
            <DataDisplayPage key={dataDisplayPageKey} folderpath={folderPath} fielddata={fieldData}
                ...
                EmptyComponent={(folderPath.length===1 && listData.length===0 )? EmptyComponent : null}
